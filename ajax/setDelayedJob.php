...
switch($objectType){
    case 'leads':
        ...
        $leadSql = " SELECT id, phone, name FROM leads WHERE " . $where . 'ORDER BY id ASC';
        $viewName = 'vw_leads_job_list_' . $currentTimeViewName; 
        ...
        if ($no_of_records) {
            ...
            $dbObj->dataInsert($arrViewInfo, 'delayed_jobs');    
            $result['success'] = true;
    ...
    case 'adm':
        // Creating a view
        $where = " status != '0' AND";
        $where .= getAdmissionFilterSQl($postedData['filter_query']);
        $admSql = " SELECT a_id AS id, phone, name FROM admission WHERE " . $where . 'ORDER BY a_id ASC';
        $viewName = 'vw_adm_job_list_' . $currentTimeViewName; 
        $leadsViewSql = "CREATE VIEW " . $viewName . ' AS ' . $admSql;
        //echo $leadsViewSql;
        //exit();
        mysql_query($leadsViewSql);
        // Creating a view
